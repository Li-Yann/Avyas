...
class UserController extends Controller
{
    ...
    {
        $getCategories = Category::withcount('quizzes')->get();
        return view('user-categories', ['getCategories' => $getCategories]);
    }

    function userShowQuizList($id, $category)
    {
        $quizList = Quiz::withCount('Mcq')->where('category_id', $id)->get();
        return view('user-list', ["quizList" => $quizList, "category" => $category]);
    }
    ...
    function userSignup(Request $request)
    {
        ...
        if ($user) {
            ...
            if (Session::has('quiz-url')) {
                ...
                return redirect($url)->with('message-success', "User Registered");
            } else {
                return redirect('/')->with('message-success', "Registered Successfully, Check Email for verification");
            }
        }
    }
    ...
    function userLogin(Request $request)
    {
        ...
        if (!$user || !Hash::check($request->password, $user->password)) {
            return redirect('user-login')->with('message-error', "Invalid email and password");
        }
    ...
    function UserMcq($id, $name)
    {
        ...
        if ($record->save()) {
            ...
            $mcqData = MCQ::find($id);
            return view('user-mcq-page', ['quizName' => $name, 'mcqData' => $mcqData]);
        } else {
    ...
    function submitMcq(Request $request, $id)
    {
        ...
        Session::put('currentQuiz', $currentQuiz);
        if ($mcqData) {
            return view('user-mcq-page', ['quizName' => $currentQuiz['quizName'], 'mcqData' => $mcqData]);
        } else {
